		...
		if(csf.getSeries_id().equalsIgnoreCase("0")){//Add new series
			try{
				String sql = "INSERT INTO series " +
						"(name,description,lastupdate,user_id,semester,url,owner_id) " +
						"VALUES (?,?, now(),?,?,?,?);";
				...
				pstmt.setLong(4, Long.parseLong(csf.getSemester()));
				pstmt.setString(5,csf.getUrl());
				pstmt.setLong(6, ub.getUserID());
				pstmt.execute();
			...
			try{
				...
				sql = "UPDATE series SET name=?,description=?,lastupdate=NOW(),user_id=?,semester=?,url=? " +
						"WHERE series_id=?";
				pstmt = conn.conn.prepareStatement(sql);
				pstmt.setString(1,csf.getName());
				pstmt.setString(2,csf.getDescription());
				...
				pstmt.setLong(4, Long.parseLong(csf.getSemester()));
				pstmt.setString(5, csf.getUrl());
				pstmt.setLong(6, Long.parseLong(csf.getSeries_id()));
				pstmt.execute();
				pstmt.close();
