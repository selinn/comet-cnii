...
//=================================================================
	...
	if(user_id_value !=null){//User Mode
		...
		if(req_posted){
			sql += "JOIN colloquium cc ON c.col_id=cc.col_id AND cc.owner_id IN (" + user_ids + ") ";
		}else{
			sql += "JOIN userprofile up WHERE c.col_id=up.col_id AND up.user_id IN (" + user_ids + ") ";
		}
	}
	if(comm_id_value != null){//Community Mode
		...
		sql += "JOIN userprofile up ON c.col_id=up.col_id JOIN contribute ct ON up.userprofile_id = ct.userprofile_id AND ct.comm_id IN (" + comm_ids + ") ";
	}
	if(tag_id_value != null){//Tag Mode
		...
		sql += "JOIN userprofile up ON c.col_id=up.col_id JOIN tags tt ON tt.userprofile_id = up.userprofile_id AND tt.tag_id IN (" + tag_ids + ") ";
	}
	if(series_id_value != null){//Series Mode
		...
		sql += "JOIN seriescol sc ON c.col_id = sc.col_id AND sc.series_id IN (" + series_ids + ") ";
	}
	if(entity_id_value != null){//Entity Mode
		String entity_ids = null;
		for(int i=0;i<entity_id_value.length;i++){
			if(entity_ids == null){
				entity_ids = entity_id_value[i];
			}else{
				entity_ids += "," + entity_id_value[i];
			}
		}
		sql += "JOIN entities ee ON c.col_id = ee.col_id AND ee.entity_id IN (" + entity_ids + ") ";
	}
	if(type_value != null){//Entity Type Mode
		...
		sql += "JOIN entities eee ON c.col_id = eee.col_id JOIN entity e ON " +
		"e.entity_id = eee.entity_id AND e._type IN (" + types + ") ";
	}
	sql += "WHERE TRUE ";
