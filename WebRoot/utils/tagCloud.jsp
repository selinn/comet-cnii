...
<% 
	...
	ResultSet rs = null;
	String sql = "SELECT tag_id,tag,COUNT(*) _no FROM (SELECT t.tag_id,t.tag,u.col_id " +
					"FROM tag t,tags tt,userprofile u " +
					"WHERE t.tag_id = tt.tag_id AND " +
					"u.col_id IN (SELECT col_id FROM colloquium WHERE _date >= (SELECT beginterm FROM sys_config) AND _date <= (SELECT endterm FROM sys_config)) AND " +
					"tt.userprofile_id = u.userprofile_id";
	...
	if(tag_id > 0){
		sql += " AND t.tag_id<>" + tag_id + " AND u.userprofile_id IN (SELECT userprofile_id FROM tags WHERE tag_id=" + tag_id + ")";
	}
	if(col_id > 0){
		sql += " AND u.col_id=" + col_id; 
	}
	if(user_id > 0){
		sql += " AND u.user_id=" + user_id;
	}
	if(comm_id > 0){
		sql += " AND u.userprofile_id IN " +
				"(SELECT userprofile_id FROM contribute " +
				"WHERE comm_id=" + comm_id + ")";
	}
	if(series_id > 0){
		sql += " AND u.col_id IN (SELECT col_id FROM seriescol WHERE series_id=" + series_id + ")";
	}
	if(entity_id_value != null){
		sql += " AND u.col_id IN (SELECT col_id FROM entities WHERE entity_id IN (" + entity_id_list + "))";
	}
	if(type_value != null){
		sql+=" AND u.col_id IN (SELECT ee.col_id FROM entities ee,entity e " +
				"WHERE ee.entity_id = e.entity_id AND e._type  IN (" + type_list + "))"; 
	}
	sql += " GROUP BY t.tag_id,t.tag,u.col_id) _t GROUP BY tag_id,tag " +
